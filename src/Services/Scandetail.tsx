...
//   pid: string;
//   data: string;
//   decodedFaultArray: Record<string, string>; // key: fault code (like P22D9), value: status (like "History/Healed")
//   header: string;
//   protocol: string;
//   system: string;
// }
...
//                 )}
//               </td>
//                 {/* <td className="border px-4 py-2">{<ScanDcodeArr/>|| 'N/A'}</td> */}
//             </tr>
//           ))}
//         </tbody>
//       </table>
...
interface ScanItem {
  ...
  app_version: string;
  pdf_report: string;
  scanResArray: any[] | null;
  decodedArray: any[] | null;
}
  ...
  return (
    <div className="p-4">
      ...
      <table className="min-w-full bg-white border border-gray-200 text-sm">
        <thead>
          <tr>
            {['Email', 'Start_time', 'End_time', 'Model', 'License_plate', 'Vin', 'Scan_ended', 'Make', 'Country', 'Type', 'App_version', 'Pdf_report', 'Show'].map((col) => (
        ...
        <tbody>
          {scans.map((scan) => (
            <tr key={scan.id}>
              ...
              <td className="border px-4 py-2">
                {scan.pdf_report ? (
                  <a href={scan.pdf_report} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                    View Report
                  </a>
                ) : 'No Report'}
              </td>
              <td className="border px-4 py-2">
                <button
                  ...
                  onClick={() => navigate(`/scans/${scan.id}`, { state: { scan } })}
                >
                  View Details
                </button>
              </td>
