...
const FaultCodeSymptoms = () => {
    ...
    const fetchFaultData = async (
        ...
        console.log('🔍 fetchFaultData called with:', { targetPage, searchDtc, searchMake, searchGeneric });
        ...
        try {
            ...
            const url = `${API_BASE_URL}api/FaultCodeSymptoms?${params.toString()}`;
            ...
            if (result && Array.isArray(result.data)) {
                ...
                if (result.data.length === 0) {
                    setError(searchDtc || searchMake || searchGeneric !== "" 
                        ? "No fault codes found for the specified criteria." 
            ...
            console.error("❌ Error fetching fault data:", err);
            setFaultData([]);
            setTotal(0);
            setError("Failed to fetch fault code data. Please try again.");
        } finally {
            setLoading(false);
            abortControllerRef.current = null;
            console.log('✨ fetchFaultData completed');
        }
    };
            ...
            <div className="bg-white border rounded-lg shadow-sm p-6 mb-6">
                <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <input
                        ...
                        placeholder="DTC Code (e.g., P1646)"
                ...
                {loading ? (
                    <div className="flex items-center justify-center py-20">
                        <div className="text-center">
                            ...
                            <p className="text-blue-500 text-xl">Loading fault code data...</p>
                                ...
                                <tbody>
                                    {faultData.map((item:any, idx) => (
                                        <tr key={item.id || idx} className="hover:bg-gray-50">
                                            ...
                                            <td className="border px-3 py-2 font-mono font-semibold text-blue-600">
                                                {item.dtc || "-"}
                                            </td>
                                            <td className="border px-3 py-2">
                                                {item.symptom || "-"}
                    ...
                    <div className="text-center py-20">
                        <p className="text-gray-500 text-xl">No fault code data to display</p>
            ...
            {total > 0 && (
                <div className="text-center text-sm text-gray-500 mt-2 bg-white rounded-lg p-4 border">
                    <div className="flex flex-wrap justify-center items-center gap-6">
                        ...
                        <span>🔧 Current Page Items: {faultData.length}</span>
                    </div>
                </div>
            )}
        </div>
...
export default FaultCodeSymptoms;